...
def main():
    ...
    if opt.task == 'non_celltype_GRN':
        if opt.setting == 'default':
            ...
            opt.batch_size = 64
        model = non_celltype_GRN_model(opt).to(device)
        if opt.setting == 'test':
            opt.beta = 1
            opt.alpha = 100
            opt.K1 = 1
            ...
            opt.lr_step_size = 0.99
            opt.batch_size = 64
            model = test_non_celltype_GRN_model(opt).to(device)
        model.train_model()
        ...
        if opt.setting == 'default':
            ...
            opt.lr_step_size = 0.99
            opt.batch_size = 64
        model = celltype_GRN_model(opt).to(device)
        if opt.setting == 'test':
            opt.beta = 0.01
            opt.alpha = 1
            opt.K1 = 1
            ...
            opt.lr_step_size = 0.99
            opt.batch_size = 64
            model = test_celltype_GRN_model(opt).to(device)
        model.train_model()
